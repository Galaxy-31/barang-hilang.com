...
@section('content')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('admin.menemukans.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
    ...
    <div class="card shadow mb-4">
        <div class="card-body" style="height: calc(100vh - 210px);">
            <h2 class="card-title mb-2">Menemukan</h2>
            ...
            <table class="table table-striped-column">
                ...
                @foreach ($menemukans as $menemukan)
                    <tr>
                        <td>{{ ++$i }}</td>
                        <td>{{ $menemukan->nama }}</td>
                        <td>{{ $menemukan->email }}</td>
                        <td>{{ $menemukan->tipe }}</td>
                        <td><img src="{{ $menemukan->foto }}" alt=""></td>
                        <td>{{ $menemukan->keterangan }}</td>
                        <td>
                            <form action="{{ route('admin.menemukans.destroy',$menemukan->id) }}" method="POST">
    
                                <a class="btn btn-info btn-sm text-white mb-2" href="{{ route('admin.menemukans.show',$menemukan->id) }}">
                                    <i class="far fa-eye"></i>
                                    Tampilkan
                                </a>
    
                                <a class="btn btn-primary btn-sm text-white mb-2" href="{{ route('admin.menemukans.edit',$menemukan->id) }}">
                                ...
                                <button type="submit" onclick="return confirm('Apakah kamu yakin ingin menghapus {{ $menemukan->nama }}?')" class="btn btn-danger btn-sm text-white mb-2">
